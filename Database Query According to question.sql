...
select billing_city,count(total)  from invoice
...
select customer.first_name, customer.last_name,sum(invoice.total) as total_spend  from  customer join invoice
...
select artist.name,count(artist.artist_Id) as top from track 
...
-- there are two mathod to run this query 
--mathod 1
...
--there are two mathod to do this 
--mathod 01 
